...
var positions        = new Float32Array(amountOfCircles * 3);
var colourAttributes = new Float32Array(amountOfCircles * 3);
var sizes            = new Float32Array(amountOfCircles);

// Renderer Initialization
renderer.setPixelRatio(window.devicePixelRatio > 1 ? 2 : 1);
renderer.setSize(width, height);
renderer.setClearColor(0x000000, 0.70);
...
for (var i = 0; i < amountOfCircles; i++) {
  ...
  colors[vector.color].toArray(colourAttributes, i * 3);
  sizes[i] = 5;
}
...
var attributeColors = new THREE.BufferAttribute(colourAttributes, 3);
...
function render(a) {
  ...
  if (intersections.length) {
    for (var i = 0; i < intersections.length; i++) {
      ...
      hovered.push(index);
      if (prevHovered.indexOf(index) === -1) onDotHover(index);
    }
  }

  for (var i = 0; i < prevHovered.length; i++) {
    if (hovered.indexOf(prevHovered[i]) === -1) mouseOut(prevHovered[i]);
  }
